...
let tinyUrls = [];
for (let i = 0; i < Math.min(resultFix.length, 2); i++) {
    let tiny = await axios.get(`https://tinyurl.com/api-create.php?url=${resultFix[i].video}`);
    tinyUrls.push(tiny.data);
}
...
let responseData = {
    title,
    tinyUrls,
    quality: resultFix.map(item => item.quality),
    thumb,
    channel,
    uploadDate,
    desc
