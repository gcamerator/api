    ...
    try {
      ...
      truecallerjs.search(searchData)
        .then(response => {
          ...
          let type = numberType.replace(/MOBILE/g, "هاتف محمول").replace(/FIXED_LINE/g, "هاتف ثابث");
          let country_Code = countryCode.replace(/MA/g, "المغرب");
          let emaill = email.replace(/unknown email/g, "غير معروف");
          let sim = carrier.replace(/Orange/g, "أورونج").replace(/Maroc Telecom/g, "اتصالات المغرب").replace(/Wana/g, "إينوي");
          const result = {
            name: name,
