...
function salat(city) {
  return new Promise((resolve, reject) => {
    fetchPrayersData(city)
      .then(data => {
        const parsedData = mapResponseToMonthlyPrayers(data);
...
function mapResponseToMonthlyPrayers(data) {
  ...
  return Array.from(items).map(item => {
    ...
    PRAYER_VALUES.forEach((value, index) => {
      if (index === 2) {
        result[value] = children[index].textContent.trim();  
      } else {
        result[value] = children[index].textContent.trim();
      }
    });
...
async function fetchPrayersData(city) {
  try {
    const url = `${API_URL}?ville=${encodeURIComponent(city)}`;  
