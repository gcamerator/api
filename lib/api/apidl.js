...
function salat(city) {
  return new Promise((resolve, reject) => {
    const cityCode = encodeURIComponent(city);
    axios.get(`https://habous.gov.ma/prieres/horaire_hijri_2.php?ville=${cityCode}`)
      .then(response => {
        const data = response.data;
        const data1 = mapResponseToMonthlyPrayers(data);
...
function fbdown(){
  return new Promise((resolve,reject) => {
    let hh = 'https://fb.watch/qwnEv8pI2W/'
  let config = {
    'url': hh
    }
  ...
  .then(async({ data }) => {
    const $ = cheerio.load(data)	
    const Normal_video = $('.card .row .col-md-4.btns-download p:nth-child(1) a').attr('href');
