...
async function markoub(city1, city2, seats, date) {
  try {
   ...
   const day = today.getDate().toString().padStart(2, '0');
    if (!seats) {let seats = 1;}
    if (!date) {let date = day;}
    const url = `https://www.markoub.ma/fr/Recherche/Show?villeDepartId=${city1}&villeArriveeId=${city2}&nbrSeats=${seats}&dateDepart=${my}-${date}`;
