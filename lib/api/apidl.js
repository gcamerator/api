      ...
      .then(html => {
        ...
        for (let i = 0; i < downloadLinks.length; i++) {
          ...
          downloadPromises.push(
            fetch(fullDownloadLink)
              .then(downloadResponse => {
                if (downloadResponse.ok) {
                  return downloadResponse.buffer();
                } else {
                  console.error(`فشل تحميل الملف: ${fullDownloadLink}`);
                  return null;
                }
              })
              .then(fileContent => ({ fileContent, title: titles[i] }))
          );
        }

        Promise.all(downloadPromises)
          .then(downloads => {
            const result = JSON.stringify(downloads.filter(download => download !== null));
            resolve(result);
          })
          .catch(err => {
            reject(err);
