...
async function markoub(city1, city2, seats, date) {
  ...
  try {
    ...
    const city1 = cities[city1];
    const city2 = cities[city2];


    const today = new Date();
    const my = `${today.getFullYear()}-${(today.getMonth() + 1)
      .toString()
...
if (!date || (!seats && !date)) {
    ...
    const url = `https://www.markoub.ma/fr/Recherche/Show?villeDepartId=${city1}&villeArriveeId=${city2}&nbrSeat=${seats}&dateDepart=${my}-${date}`;
