...
const API_URL = 'https://saveig.app/api/ajaxSearch';
...
function parseResponse(response) {
  const $ = cheerio.load(response.data);
  const urls = $('div.download-items__btn a')
...
function igdl(url) {
  return new Promise((resolve, reject) => {
    fetchData(url)
      .then(response => {
        const urls = parseResponse(response);
        resolve(urls);
      })
      .catch(error => {
        reject(new Error('Failed to get download URLs'));
      });
  });
}
