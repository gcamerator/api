...
const getLiveMatches = require('./scraper/getLiveMatches');
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      matchesInfo.forEach(match => {
        ...
        // Check if the league exists in organizedData
        if (!organizedData.hasOwnProperty(leagueName)) {
          // If not, initialize it with an empty array
          organizedData[leagueName] = [];
        }

        // Push the match data to the corresponding league
        organizedData[leagueName].push(matchData);
      });
...
function truecaller(num, id) {
  return new Promise((resolve, reject) => {
    try {
      ...
      truecallerjs.search(searchData)
        .then(response => {
...
let phoneType = numberType.replace(/MOBILE/g, "هاتف محمول").replace(/FIXED_LINE/g, "هاتف ثابث"); // تم تغيير اسم المتغير
let countryName = countryCode.replace(/MA/g, "المغرب"); // تم تغيير اسم المتغير
let emailContent = email.replace(/unknown email/g, "غير معروف"); // تم تغيير اسم المتغير
let carrierName = carrier.replace(/Orange/g, "أورونج").replace(/Maroc Telecom/g, "اتصالات المغرب").replace(/Wana/g, "إينوي");
const result = {
  name: name,
  ...
  email: emailContent
        ...
        .catch(error => {
        reject(`Failed to get data: ${error.message}`);
        });
    } catch (error) {
...
function salat() {
  return new Promise((resolve, reject) => {
    const PRAYER_VALUES = ['Fajr', 'Sunrise', 'Dhuhr', 'Asr', 'Maghrib', 'Isha'];
    
    try {
      const url = `https://habous.gov.ma/prieres/horaire_hijri_2.php`;
      axios.get(url)
        .then(response => {
          const $ = cheerio.load(response.data);
          
          const prayersData = [];
          $('#horaire tbody tr').each((i, element) => {
            const parsedData = {};
            const children = $(element).find('td');
            PRAYER_VALUES.forEach((value, index) => {
              parsedData[value] = $(children[index]).text().trim();
            });
            prayersData.push(parsedData);
          });
    
          const results = JSON.stringify(prayersData);
          resolve(results);
        })
        .catch(error => {
          reject(new Error(`Failed to get prayers data: ${error.message}`));
        });
    } catch (error) {
      reject(new Error(`Failed to get prayers data: ${error.message}`));
    }
...
// markoub
function markoub(city1, city2, seats, date) {
  return new Promise((resolve, reject) => {
    const cityCode1 = cities[city1];
    const cityCode2 = cities[city2];
    const url = `https://www.markoub.ma/fr/Recherche/Show?villeDepartId=${cityCode1}&villeArriveeId=${cityCode2}&nbrSeats=${seats}&dateDepart=${date}`;
...
function hespress() {
  return new Promise((resolve, reject) => {
    fetch('https://www.hespress.com/all')
  ...
  url: decodeURIComponent($(element).find('.stretched-link').attr('href')) 
};
results.push(card);
});
...
function hesport() {
  return new Promise((resolve, reject) => {
    fetch('https://www.hesport.com/all')
      .then(response => response.text())
      .then(html => {
        const $ = cheerio.load(html);
        const results = [];
      ...
      const card = {
        ...
        link: decodeURIComponent($(element).find(".stretched-link").attr("href"))
      };
      results.push(card);
    });
const result = JSON.stringify(results);
 ...
 return new Promise((resolve, reject) => {
    fetch(url) 
      ...
      .then(html => {
        ...
        let content = $('.article-content').text().trim().replace(/\./g, '.\n\n');

        if (!title || !content) {
          reject("لم يتم العثور على محتوى المقالة.");
        } else {
          const article = {
            title,
            image,
...
// dtalamidi
function dtalamidi(url) {
  return new Promise((resolve, reject) => {
    fetch(url)
      .then(response => response.text())
      .then(html => {
        const $ = cheerio.load(html);
        const link = $("table#t01 tr td:nth-child(2) a")
          .map((i, el) => 'https://talamidi.com' + decodeURIComponent($(el).attr("href")))
          .get();
        const title = $("table#t01 tr td:nth-child(1)")
...
function hiba() {
  return new Promise((resolve, reject) => {
    fetch('http://ar.hibapress.com/akhbar.php')
      .then(response => response.text())
      .then(html => {
        const $ = cheerio.load(html);
        const results = [];
    $("ul#latest_posts li").each((index, element) => {
      const card = {
        ...
        link: decodeURIComponent($(element).find("div.lastNewsText a").attr("href"))
      };
      results.push(card);
    });
const result = JSON.stringify(results);
...
function fbdown(url){
  ...
  axios('https://www.getfvid.com/downloader',{
      ...
      headers: {
        "content-type": "application/x-www-form-urlencoded",
        "user-agent":  "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36",
        "cookie": "_ga=GA1.2.102944526.1709221032; _pbjs_userid_consent_data=3524755945110770; cto_bidid=rQH5Tl9NNm5IWFZsem00SVVuZGpEd21sWnp0WmhUeTZpRXdkWlRUOSUyQkYlMkJQQnJRSHVPZ3Fhb1R2UUFiTWJuVGlhVkN1TGM2anhDT1M1Qk0ydHlBb21LJTJGNkdCOWtZalRtZFlxJTJGa3FVTG1TaHlzdDRvJTNE; cto_bundle=g1Ka319NaThuSmh6UklyWm5vV2pkb3NYaUZMeWlHVUtDbVBmeldhNm5qVGVwWnJzSUElMkJXVDdORmU5VElvV2pXUTJhQ3owVWI5enE1WjJ4ZHR5NDZqd1hCZnVHVGZmOEd0eURzcSUyQkNDcHZsR0xJcTZaRFZEMDkzUk1xSmhYMlY0TTdUY0hpZm9NTk5GYXVxWjBJZTR0dE9rQmZ3JTNEJTNE; _gid=GA1.2.1614945873.1709221032; __gads=ID=5be9d413ff899546-22e04a9e18ca0046:T=1625126836:RT=1625126836:S=ALNI_Ma0axY94aSdwMIg95hxZVZ-JGNT2w; cookieconsent_status=dismiss"
      }
    })
  .then(async({ data }) => {
    const $ = cheerio.load(data)	
    const sd = $('.card .row .col-md-4.btns-download p:nth-child(1) a').attr('href');
...
module.exports.kora = kora
module.exports.cook = cook
module.exports.truecaller = truecaller
module.exports.dtalamidi = dtalamidi
module.exports.talamidi = talamidi
module.exports.salat = salat
module.exports.hiba = hiba
module.exports.rhiba = rhiba
module.exports.hespress = hespress
module.exports.rhespress = rhespress
module.exports.hesport = hesport
module.exports.markoub = markoub
